...
class Procedure:
    ...
    def run(
        self,
        file: CTDData | CnvFile | HexFile | Path | str = "",
    ) -> CTDData:
        """
        ...
        if isinstance(file, CnvFile):
            ...
            self.target = file.path_to_file
            self.load_config()
            self.ctd_data = decode_hex(file, downcast_only=self.downcast_only)
            ...
            if first_module.lower() in self.available_hex_converters:
                ...
                if first_module.lower() == "datcnv":
                    self.ctd_data = self.convert(
                        self.target, {first_module: hex_converter}
                    )
                elif first_module.lower() == "hex2py":
                    self.ctd_data = decode_hex(
                        self.target,
                        downcast_only=self.downcast_only,
                        **hex_converter,
                    )
            else:
    ...
    def create_seabird_step(
        ...
        ----------
        module : ProcessingModule
            A module to create a step from
        input_path : Path
            Path to target file
        output_name: Path | str | None = None
    ...
    def handle_fill_type_dir_creation(self):
        """Move target files to corresponding file type directories."""
